...
export function buildRequestBuckets(entries: AuditEntry[]): RequestBucket[] {
  ...
  entries.slice(0, 180).forEach((entry, entryIndex) => {
    const bucketIndex = bucketCount - 1 - Math.min(bucketCount - 1, Math.floor(entryIndex / 4));
...
export function useAuditMetrics(entries: AuditEntry[]): AuditMetrics {
  return useMemo(() => {
    ...
    const durations = entries.map((e) => Number(e.duration_ms)).filter((v) => Number.isFinite(v));
    const avgDuration = durations.length
      ? Math.round(durations.reduce((sum, v) => sum + v, 0) / durations.length)
      : 0;

    const successCount = entries.filter((e) => e.status_code >= 200 && e.status_code < 300).length;
