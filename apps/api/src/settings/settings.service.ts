...
export interface SettingRecord { key: string; value: string; updated_at: string }
...
export class SettingsService {
  ...
  async getSetting(key: string): Promise<SettingRecord | null> {
    ...
    const request = this.prisma.setting.findUnique({ where: { key } }).then((row) => row ? { key: row.key, value: row.value, updated_at: row.updatedAt.toISOString() } : null);
    this.inflight.set(key, request);
    try {
      const value = await request;
      if (this.inflight.get(key) === request) this.cache.set(key, { value, expiresAt: Date.now() + this.ttl });
      return value;
  ...
  async listSettings(): Promise<SettingRecord[]> {
    ...
    return rows.map((row) => ({ key: row.key, value: row.value, updated_at: row.updatedAt.toISOString() }));
  }

  async setSetting(key: string, value: string): Promise<SettingRecord> {
    const row = await this.prisma.setting.upsert({ where: { key }, create: { key, value }, update: { value } });
  ...
  async deleteSetting(key: string): Promise<boolean> {
    try { await this.prisma.setting.delete({ where: { key } }); this.cache.delete(key); return true; }
  ...
  async getDefaultRouteMode(fallback: RouteMode): Promise<RouteMode> {
    ...
    return setting && (VALID_ROUTE_MODES as readonly string[]).includes(setting.value) ? setting.value as RouteMode : fallback;
  }
