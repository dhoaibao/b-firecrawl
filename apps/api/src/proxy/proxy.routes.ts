...
export class ProxyRoutes implements OnModuleInit {
  ...
  onModuleInit(): void {
    ...
    fastify.all("/v1/*", (request: FastifyRequest, reply: FastifyReply) =>
      this.proxy.handle(request as RequestWithContext, reply),
    );
    fastify.all("/v2/*", (request: FastifyRequest, reply: FastifyReply) =>
      this.proxy.handle(request as RequestWithContext, reply),
    );
    fastify.all("/admin/api/playground/v1/*", (request: FastifyRequest, reply: FastifyReply) =>
      this.playground(request, reply),
    );
    fastify.all("/admin/api/playground/v2/*", (request: FastifyRequest, reply: FastifyReply) =>
      this.playground(request, reply),
    );
  }
